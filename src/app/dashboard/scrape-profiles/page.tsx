...
export default function ScrapeProfilesPage() {
  ...
  const statusConfig = {
    idle: { icon: Clock, color: "text-gray-400", bg: "bg-gray-50", label: "Idle" },
    ...
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Configure web scraping sources for your leads.</p>
        </div>
        <button onClick={() => setShowModal(true)} className="flex items-center gap-2 bg-gray-900 text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-gray-700 transition-colors">
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center shadow-sm">
          ...
          <h2 className="font-bold text-gray-900 mb-2">No scrape profiles yet</h2>
          <p className="text-sm text-gray-500 mb-5">Define a web source to extract leads automatically.</p>
        ...
        <div className="grid gap-4">
          {profiles.map(profile => {
            ...
            return (
              <div key={profile.id} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${cfg.bg} ${cfg.color} font-medium`}>
                        <StatusIcon size={11} className={profile.status === "running" ? "animate-pulse" : ""} />
                        {cfg.label}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 truncate mb-2">{profile.url}</p>
                    <div className="flex items-center gap-4 text-xs text-gray-400">
                      <span>{profile.resultsCount} results</span>
                      {profile.lastRun && <span>Last run: {new Date(profile.lastRun).toLocaleDateString()}</span>}
                      {profile.schedule && <span>Schedule: {profile.schedule}</span>}
                    </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-5">Add Scrape Profile</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Profile Name</label>
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" placeholder="Tech Leads" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
              </div>
              <div>
