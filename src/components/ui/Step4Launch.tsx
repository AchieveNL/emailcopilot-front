...
import { ArrowLeft, Rocket, Settings, Mail, Database, Clock, Calendar } from "lucide-react";
      ...
      <div className="space-y-4 mb-8">
        ...
        <div className="border border-gray-100 rounded-xl p-4 bg-gray-50">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <span className="font-medium text-gray-900">{copilotData.settings.dailyLimit} emails/day</span>
            </div>
            <div>
              <span className="text-xs text-gray-400 block mb-0.5">Sending Speed</span>
              <span className="font-medium text-gray-900">{copilotData.settings.sendingSpeed}</span>
            </div>
            <div>
              <span className="text-xs text-gray-400 block mb-0.5">Time Zone</span>
              <span className="font-medium text-gray-900 text-xs">{copilotData.settings.timezone}</span>
            </div>
          </div>
        </div>
          ...
          {selectedEmailProfile && (
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full bg-gray-900 text-white flex items-center justify-center text-xs font-bold">
                {selectedEmailProfile.name.split(" ").map((n) => n[0]).join("").slice(0, 2)}
              </div>
              <div>
                <div className="text-sm font-medium text-gray-900">{selectedEmailProfile.name}</div>
                <div className="text-xs text-gray-500">Profile ID: {selectedEmailProfile.id}</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="border border-gray-100 rounded-xl p-4 bg-gray-50">
          ...
          {selectedScrapeProfile && (
            <div>
              <div className="text-sm font-medium text-gray-900">{selectedScrapeProfile.name}</div>
              <div className="text-xs text-gray-500 mt-0.5">Profile ID: {selectedScrapeProfile.id}</div>
            </div>
          )}
        </div>
        ...
        <div className="border border-gray-100 rounded-xl p-4 bg-blue-50">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="text-xl font-bold text-gray-900">{parseInt(copilotData.settings.dailyLimit.toString()).toLocaleString()}</div>
